...
# Define our own url_decode and url_parse functions since they're not available in this Werkzeug version
def url_decode(qs):
    """Parse a query string and return a dictionary of values."""
    if not qs:
        return {}
    return dict(parse_qsl(qs))

def url_encode(obj, **kwargs):
    """Encode a dictionary or sequence of two-element tuples into a query string."""
    if not obj:
        return ""
    return urlencode(obj, **kwargs)
...
def login_url(login_view, next_url=None, next_field="next"):
    ...
    parsed_result = parsed_result._replace(
        netloc=netloc, query=url_encode(md, doseq=True)
    )
